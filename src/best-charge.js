...
function buildReceiptString(receipt) {
  ...
  for (let {name, price, count, payPrice} of receipt.receiptItems) {
  ...
  if (receipt.chosenType !== '') {
    ...
    if (receipt.chosenType === '指定菜品半价') {
      let lin = [];
      for (let {name, saved} of receipt.receiptItems) {
        if (saved > 0)
          lin.push(`${name}`);
      }
      lines.push(`指定菜品半价(${lin.join('，')})，省${receipt.totalSaved}元`);
    } else {
  ...
  let text = lines.join('\n');
  require('fs').writeFileSync('1.txt', text);
  return text;
}
